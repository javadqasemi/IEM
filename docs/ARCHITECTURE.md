...
**A second deliberate exception: the 3D scene in `#ablauf`.** Its ten media carry the *planner's* colours, read off `IfcStyledItem` in the client's own IFC — saturated red, blue, cyan, violet, yellow, green, brown. These are not tokens and must not become any; they are the HLK convention as the model states it, the way an IFC viewer shows the same file. The user chose this on 3 September 2026 over the muted navy/gold set, knowing it departs from the rule below. It applies to that scene only — the hero section drawing still uses `disc-*`. See "The Ablauf scene" for the full table and the two colour collisions that are in the source.
...
- **The geometry is generated and shared with `#ablauf`.** Both call `baueModell` in `src/lib/modellSzene.ts`, which builds the 7'786 components as instanced meshes. Only the *staging* differs: three acts with figures and the design-value readouts there, six phases and silence here. Never type a dimension into either component.
...
- **Nothing floats over the model any more, and the machinery for it is gone.** The plant-room names ("Heizzentrale", "Lüftungszentrale") and the three design values (heating load, flow/return temperature, supply-duct size) were removed at the client's request (September 2026), along with the project name in the title block. With them went the anchor projection: a `MutationObserver` collecting `[data-anchor]` nodes and a per-frame pass writing each one's screen transform. The only text over the canvas now is the media legend and the title block, both fixed to the frame rather than pinned to the model. **The data is untouched** — `zentralen` still frames the camera, `messpunkte` still comes out of the property sets — so reinstating labels is a rendering job, not a regeneration. Ask before doing it.
...
- **Three representations, and the reason for each.** Round pipes become a cylinder from axis and radius — a pipe *is* a cylinder, so this is the same description written shorter. Rectangular ducts become a box along the axis, with width, height and the cross-axis from the port placement. Everything else becomes an oriented box fitted to the component's own footprint (smallest enclosing rectangle, not the axis-aligned box, so a wall standing at an angle is that wall). **Slabs are the exception and stay a real triangle mesh:** a box over a floor slab fills the shafts and swallows the risers the scene exists to show. 18 slabs are 472 triangles.
- **A fitting is not one cylinder, and this was fixed the hard way.** An earlier version ran a single cylinder through *every* pipe component, between its two furthest-apart ports. Measured against the source that was wrong four ways: 1'691 heating fittings are 89 % real 90° bends and became chords cutting the corner; 255 components carry three or four ports and lost their branch entirely; 694 rectangular ducts were drawn round with `hypot(s1,s2)/4` as the radius, commented "area-equivalent" — a 500 × 500 duct came out at 39 % of its cross-section; and 2'110 fittings fell back to a 30 mm radius because the reader only knew `Tech-Outside-diameter`, while those fittings carry their width under `Tech-DN` and `Tech-DN1/2/3`. **The data was there and was not read.** Now every `IfcDistributionPort` also contributes its direction (the local z of its placement); the port axes of a fitting meet in its centre, and each port gets its own leg from there. Two ports become the bend, whose curvature radius follows from the tangent length. Measured p95 deviation of the real mesh from the drawn body, median: duct segments 0.107 m → 0.000, duct fittings 0.185 m → 0.060, silencers 0.107 m → 0.025. **Re-run that check if you touch the pipe rules.**
- **There is no analytic pipe geometry in these files — the mesh is the source of truth.** Every pipe and duct component is an `IfcFaceBasedSurfaceModel` inside a `MappedRepresentation`: no `IfcSweptDiskSolid`, no profile, no directrix. So axis-plus-radius is always a *reconstruction*, and the honest check is to measure it back against the mesh. The `MappedRepresentation` offers no instancing either — 5'283 catalogue entries for 4'484 heating components, each used exactly once. Don't go looking for a shared-geometry win that isn't there.
- **The mesh includes the insulation, and the scene now draws it.** 723 heating and 305 ventilation components carry `Tech-Insulation thickness (mm)` of 20–100 mm, held in the IFC as a second representation item (hence components with 2–4 items). Insulated, a pipe is a median **2.99×** and up to **5.46×** its bare diameter — which is why the runs looked far too thin next to an IFC viewer. Insulation is emitted as its own medium (`daemmung`, index 6) so the existing renderer draws it with no special case, and it carries a `deckkraft` factor of 0.30 with `depthWrite` off, otherwise the sleeve erases the pipe inside it. **If you compare drawn geometry against the source mesh, subtract the insulation first or you will measure it as error** — that mistake cost a full round here.
- **A T-piece is not a bend, and the port count will lie to you about that.** Of 322 heating T-pieces only 201 carry three ports: about 120 carry two, and two perpendicular ports look exactly like an elbow — reconstruct naively and you round off a corner that is square and drop the through-run. Another 115 carry only the collinear through-run, so their axes never intersect and the branch vanishes entirely. `Tech-DN3` is the reliable signal for "this has three ways"; the missing leg is then recovered from the mesh (`fehlender_ast` for the cornerless-looking case, `abzweig_aus_netz` for the collinear one). Rounding is applied only when `Tech-Product type` actually says *Bogen*. Measured p95 deviation over pipe radius fell from 1.55 to **0.00** for T-pieces and 1.29 to **0.00** for reduced T-pieces; 90° bends sit at 0.11, 45° at 0.00.
- **A fitting's width is inherited through `IfcRelConnectsPorts`.** The fitting mostly does not carry its own; the pipe attached to it does. That is also what gives a reduced T-piece a different width per leg instead of one for all three. Nominal width is mapped to outside diameter through a table built from the pipes of the same file, which carry both — so DN 15 is 21.3 mm here, like the pipes beside it, not 15 mm.
- **About a sixth of the pipe components carry only one port.** Their axis is recovered from the mesh by its first principal component — a cylinder's axis is the direction of greatest spread. Without that fallback 650 pipes and 118 ducts would have dropped out of the pipe layer into generic boxes, which is the quiet kind of loss this generator exists to prevent. The radius still comes from the property set where there is one: the spread of the point cloud is far too wide on a clipped bend.
- **`inventar` counts components, `teile` counts the bodies made from them.** Since a fitting is now several bodies, these differ — 7'786 components are drawn as 10'156 bodies, plus 1'475 insulation sleeves. The card's title block says "Bauteile" and therefore reads `n`, not `teile`. Keep it that way, or the caption starts reporting pipe pieces as components.
- **Everything is instanced.** One `InstancedMesh` per body category, and per medium one for round pipes and one for rectangular ducts — about thirty draw calls for 7'786 components. One mesh each would stall the frame on the draw calls alone. An act change only animates material opacities, so no geometry is rebuilt and the transitions stay interruptible.
- **The scene shows media, not trades — ten of them, and in the model's own colours.** This is a deliberate exception to the brand palette, decided by the user on 3 September 2026, and it is the one place on the page where `disc-*`/`brand-*` do not apply. Two things changed at once:
  - **Ten systems, not five.** The files carry Vorlauf, Rücklauf, Vorlauf/Rücklauf bestehend, Kaltwasser VL and RL, Zuluft, Abluft, Außenluft, Fortluft. The scene used to fold these onto five media — Außenluft counted as Zuluft (63 components), Fortluft as Abluft (43), existing pipework as new (205). `SYSTEM_ZU_MEDIUM` in the generator now keeps them apart. It sits there and **not** in `IS.SYSTEME`, because that table still feeds the hero section drawing, which does fold them.
  - **The colours are the planner's, read off `IfcStyledItem`.** Vorlauf `#ff0000`, Rücklauf `#0000ff`, both *bestehend* `#000000`, Kaltwasser VL `#00ffff` and RL `#6600cc`, Zuluft `#ff0000`, Abluft `#ffcc00`, Außenluft `#66ff33`, Fortluft `#993300`. **Two collisions are in the source, not a bug here:** Vorlauf and Zuluft are the same red, and the two *bestehend* systems are both black. Don't "fix" either without asking — they are what the model says.
  - **Insulation is the single deviation:** grey `#c4c9d2`, where the IFC gives each insulation body its pipe's own colour. Following the source there would need a second medium per medium and a twenty-field legend.
  - Media are addressed **by id, never by index** — `ModelScene`'s desk pens learned that the hard way when the list went from five entries to twelve.
- **For the flow animation the exporter chains components into runs.** The scene draws 7'694 pipes and 1'468 ducts as separate bodies; a slug needs a *route*. Insulation sleeves are deliberately left out of the chaining — a slug must run through the pipe, not through its jacket. `fliesslinien` holds the longest chained polylines per medium — a slug that stops after 40 cm at the next fitting shows no circuit.
- **three.js and the model both load on demand.** A type-only import at the top plus `await import()` inside the effect keeps three.js in its own chunk (~190 KB gzip) and the model in a JSON asset (~117 KB gzip) against the page's own ~86 KB. An IntersectionObserver starts both when the section approaches. Don't turn either into a static import.
